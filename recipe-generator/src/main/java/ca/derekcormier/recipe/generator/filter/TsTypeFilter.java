...
public class TsTypeFilter extends RecipeFilter {
  ...
  public static String toTsType(ParamType type) {
    boolean isWrapped =
        type.isVararg()
            && type.getType() instanceof PrimitiveType
            && ((PrimitiveType) type.getType()).getPrimitive() == Primitive.STRING;
    return (isWrapped ? "(" : "")
        + _toTsType(type.getType())
        + (isWrapped ? ")" : "")
        + (type.isVararg() ? "[]" : "");
  }
  ...
  private static String _toTsType(Type type) {
    if (type instanceof PrimitiveType) {
      ...
      boolean isWrapped =
          ((ArrayType) type).getBaseType() instanceof PrimitiveType
              && ((PrimitiveType) ((ArrayType) type).getBaseType()).getPrimitive()
                  == Primitive.STRING;
      return (isWrapped ? "(" : "")
          + _toTsType(((ArrayType) type).getBaseType())
          + (isWrapped ? ")" : "")
          + "[]";
    }
    throw new RuntimeException("unknown type");
