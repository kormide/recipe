...
public class CookbookLoader {
  ...
  private void validateInitializersContainRequiredFields(Cookbook cookbook) {
    for (Ingredient ingredient : cookbook.getIngredients()) {
      Set<String> requiredParams =
          ingredient.getRequired().stream().map(r -> r.getName()).collect(Collectors.toSet());
  ...
  private void validateNoDuplicateIngredientNames(Cookbook cookbook) {
    if (cookbook.getIngredients().stream().map(Ingredient::getName).distinct().count()
        != cookbook.getIngredients().size()) {
  ...
  private void validateParamTypes(Cookbook cookbook) {
    ...
    for (Ingredient ingredient : cookbook.getIngredients()) {
      types.addAll(
          ingredient.getRequired().stream().map(Required::getType).collect(Collectors.toList()));
      types.addAll(
          ingredient.getOptionals().stream()
              .filter(o -> !o.isCompound())
              .map(Optional::getType)
              .collect(Collectors.toList()));
      types.addAll(
          ingredient.getOptionals().stream()
              .filter(Optional::isCompound)
  ...
  private void validateNoDuplicateFieldNames(Cookbook cookbook) {
    for (Ingredient ingredient : cookbook.getIngredients()) {
      ...
      names.addAll(
          ingredient.getRequired().stream().map(Required::getName).collect(Collectors.toSet()));
      names.addAll(
          ingredient.getOptionals().stream().map(Optional::getName).collect(Collectors.toSet()));

      if (names.size() != (ingredient.getRequired().size() + ingredient.getOptionals().size())) {
        throw new RuntimeException(
            "detected duplicate field names for ingredient '" + ingredient.getName() + "'");
      }
    }
  }
  ...
  private void validateInitializerSignaturesUnique(Cookbook cookbook) {
    for (Ingredient ingredient : cookbook.getIngredients()) {
      Map<String, String> requiredParams =
          ingredient.getRequired().stream()
              .collect(Collectors.toMap(Required::getName, Required::getType));

      List<List<String>> signatures = new ArrayList<>();

      for (Initializer initializer : ingredient.getInitializers()) {
        List<String> params =
            initializer.getParams().stream().map(requiredParams::get).collect(Collectors.toList());
        signatures.add(params);
      }

      if (signatures.stream().distinct().count() != signatures.size()) {
        throw new RuntimeException(
            "initializer signatures for ingredient '" + ingredient.getName() + "' are ambiguous");
      }
    }
  }
    ...
    for (Ingredient ingredient : cookbook.getIngredients()) {
      for (Required required : ingredient.getRequired()) {
        if (!required.hasDefault()
            && (ingredient.getInitializers().isEmpty()
                || !ingredient.getInitializers().stream()
                    .allMatch(i -> i.getParams().contains(required.getName())))) {
          throw new RuntimeException(
              "required '"
                  + required.getName()
                  + "' must either have a default or appear in all initializers");
        }
      }
    }
  }
  ...
  private void validateVaragParamsAppearLastInParamLists(Cookbook cookbook) {
    for (Ingredient ingredient : cookbook.getIngredients()) {
      Set<String> requiredVarargParams =
          ingredient.getRequired().stream()
              .filter(r -> CookbookUtils.parseType(r.getType(), cookbook).isVararg())
              .map(Required::getName)
      ...
      for (Optional optional : ingredient.getOptionals()) {
        if (optional.isCompound()) {
          for (int i = 0; i < optional.getParams().size() - 1; i++) {
            if (CookbookUtils.parseType(optional.getParams().get(i).getType(), cookbook)
                .isVararg()) {
              throw new RuntimeException(
                  "vararg param can only be last param in compound optional");
            }
          }
        }
      }
    ...
    for (Ingredient ingredient : cookbook.getIngredients()) {
      for (String constant : ingredient.getConstants().keySet()) {
        if (constant == null || constant.equals("") || !SourceVersion.isName(constant)) {
          throw new RuntimeException(
              "ingredient '" + ingredient.getName() + "' has invalid constant name " + constant);
        }
      }
    }
  }
    ...
    for (Ingredient ingredient : cookbook.getIngredients()) {
      if (ingredient.isKeyed()) {
        if (ingredient.getDefaultKey() != null
            && !ingredient.getRequired().stream()
                .map(Required::getName)
                .anyMatch(r -> r.equals(ingredient.getDefaultKey()))) {
          throw new RuntimeException(
              "ingredient '"
                  + ingredient.getName()
                  + "' has a default key that is not a required string parameter");
        }
      } else {
        if (ingredient.getDefaultKey() != null) {
          throw new RuntimeException(
              "ingredient '" + ingredient.getName() + "' has a default key but keyed=false");
        }
      }
    }
  }
  ...
  private void validateNoDuplicateEnumNames(Cookbook cookbook) {
    Set<String> enumNames =
        cookbook.getEnums().stream().map(Enum::getName).collect(Collectors.toSet());
